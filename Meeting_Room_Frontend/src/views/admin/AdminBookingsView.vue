<template>
  <div class="admin-view-wrapper">
    ...
    <v-app-bar flat color="transparent" class="px-md-4 border-bottom bg-glass-nav">
       ...
       <v-spacer></v-spacer>
       <v-btn to="/admin/rooms" variant="text" color="white" class="mr-2">{{ $t('admin.manageRooms') }}</v-btn>
    ...
    <v-main class="position-relative z-index-1">
      <v-container class="py-8">
        <h1 class="text-h4 font-weight-bold text-white mb-6">{{ $t('admin.manageBookings') }}</h1>

        <!-- Stats Row -->
        <v-row class="mb-6">
            <v-col cols="12" md="3">
                <v-card theme="dark" class="bg-glass-card pa-4 text-center">
                    <div class="text-overline text-medium-emphasis">{{ $t('admin.totalBookings') }}</div>
                    <div class="text-h4 font-weight-bold text-white">{{ bookings.length }}</div>
                </v-card>
            </v-col>
            <v-col cols="12" md="3">
                <v-card theme="dark" class="bg-glass-card pa-4 text-center">
                    <div class="text-overline text-warning">{{ $t('admin.pending') }}</div>
                    <div class="text-h4 font-weight-bold text-white">{{ bookings.filter(b => b.status === 'PENDING').length }}</div>
                </v-card>
            </v-col>
             <v-col cols="12" md="3">
                <v-card theme="dark" class="bg-glass-card pa-4 text-center">
                    <div class="text-overline text-success">{{ $t('admin.confirmed') }}</div>
                    <div class="text-h4 font-weight-bold text-white">{{ bookings.filter(b => b.status === 'CONFIRMED').length }}</div>
                </v-card>
            </v-col>
             <v-col cols="12" md="3">
                <v-card theme="dark" class="bg-glass-card pa-4 text-center">
                    <div class="text-overline text-error">{{ $t('admin.rejectedCancelled') }}</div>
    ...
    <v-dialog v-model="actionDialog" max-width="400" persistent>
      <v-card theme="dark" class="bg-glass" rounded="lg">
        ...
        <v-card-actions class="pa-6 pt-4">
          <v-spacer></v-spacer>
          <v-btn variant="text" @click="actionDialog = false">{{ $t('booking.cancel') }}</v-btn>
