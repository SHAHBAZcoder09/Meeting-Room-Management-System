    ...
    <nav class="nav-container">
      <div class="nav-content">
        ...
        <div class="nav-actions">
          ...
          <v-btn variant="outlined" color="white" to="/login" class="text-capitalize px-6" rounded="pill">
            {{ $t('nav.signIn') }}
          </v-btn>
        </div>
      </div>
      ...
      <div class="auth-container slide-up">
        ...
        <div class="auth-branding d-none d-lg-flex">
          <div class="branding-content">
            <h2 class="branding-title">{{ $t('auth.joinThousands') }}<br><span class="title-gradient">N4Booking</span></h2>
            ...
            <div class="trust-badges mt-8">
              <div class="badge-row">
                ...
                <span>{{ $t('auth.enterpriseSecurity') }}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <v-card theme="dark" class="auth-card" elevation="0" rounded="xl">
            <div class="text-center mb-6">
              <div class="avatar-container mb-4">
                <v-avatar color="secondary" size="72" class="glow-avatar-cyan">
                  <v-icon icon="mdi-account-plus" size="36" color="white"></v-icon>
                </v-avatar>
              </div>
              <h1 class="text-h4 font-weight-bold text-white mb-2">{{ $t('auth.createAccount') }}</h1>
              <p class="text-grey-lighten-1">{{ $t('auth.createAccountSubtitle') }}</p>
            </div>
            ...
            <v-form @submit.prevent="handleRegister" v-model="isValid">
              ...
              <v-btn
                ...
                rounded="lg"
                :loading="isLoading"
                class="text-capitalize font-weight-bold cta-btn-cyan mb-4"
                height="52"
              >
              ...
              <div class="terms-text text-caption text-center text-grey mb-4">
                {{ $t('auth.termsAgree') }} 
                <a href="#" class="text-primary">{{ $t('auth.termsOfService') }}</a> {{ $t('common.and') || 'and' }} 
                <a href="#" class="text-primary">{{ $t('auth.privacyPolicy') }}</a>
              </div>
            </v-form>
            ...
            <div class="text-center text-body-2 text-grey">
              {{ $t('auth.hasAccount') }} 
              <router-link to="/login" class="text-primary font-weight-bold text-decoration-none">{{ $t('auth.signIn') }}</router-link>
            </div>
          </v-card>
        </div>
      </div>
...
const handleRegister = async () => {
    ...
    try {
        ...
        if (response.created || response.ok) {
            ...
            showNotification('Welcome aboard! 🎉', 'success');
