<template>
  <div class="my-bookings-wrapper">
    ...
    <v-app-bar flat color="transparent" class="px-md-4 border-bottom bg-glass-nav">
       ...
       <v-btn to="/dashboard" variant="text" color="white" prepend-icon="mdi-arrow-left">{{ $t('nav.backToDashboard') }}</v-btn>
    </v-app-bar>

    <v-main class="position-relative z-index-1">
      <v-container class="py-8">
        <h1 class="text-h4 font-weight-bold text-white mb-6">{{ $t('nav.myBookings') }}</h1>
        ...
        <div v-else-if="bookings.length === 0" class="text-center py-16">
            ...
            <h3 class="text-h6 text-grey">{{ $t('myBookings.noBookings') }}</h3>
            <v-btn color="primary" to="/dashboard" class="mt-4">{{ $t('dashboard.bookRoom') }}</v-btn>
        </div>
            ...
            <v-col v-for="booking in bookings" :key="booking.id" cols="12" md="6" lg="4">
                <v-card theme="dark" class="bg-glass booking-card" rounded="lg" elevation="0">
                    ...
                    <v-card-text class="pt-2">
                        <div class="mb-2">
                            <span class="text-grey text-caption d-block">{{ $t('myBookings.time') }}</span>
                            <span class="text-white">{{ formatTime(booking.startTime) }} - {{ formatTime(booking.endTime) }}</span>
                        </div>
                        <div>
                            <span class="text-grey text-caption d-block">{{ $t('myBookings.purpose') }}</span>
    ...
    <v-dialog v-model="cancelDialog" max-width="400" persistent>
      <v-card theme="dark" class="bg-glass" rounded="lg">
        ...
        <v-card-actions class="pa-6 pt-4">
          <v-spacer></v-spacer>
          <v-btn variant="text" @click="cancelDialog = false">{{ $t('myBookings.keepBooking') }}</v-btn>
          <v-btn color="error" variant="flat" @click="confirmCancelBooking">{{ $t('myBookings.yesCancel') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
